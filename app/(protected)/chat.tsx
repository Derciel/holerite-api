...
export default function ChatScreen() {
  ...
  const { userId, nome, role } = useAuth();
  ...
  const loadMensagens = useCallback(async () => {
    ...
    const data = await fetchChatMensagens(String(userId), role);
    setMensagens(data);
    setLoading(false);
  }, [userId, role]);
  ...
  useEffect(() => {
    ...
    // Marcar como lidas
    if (userId && role) {
      markChatAsRead(String(userId), role);
    }
  ...
  }, [loadMensagens, userId, role]);

  const handleEnviar = async () => {
    if (!novaMensagem.trim() || !userId || !nome || !role || enviando) return;
  ...
  const renderItem = ({ item, index }: { item: ChatMensagem; index: number }) => {
    const isMe = item.remetente_id === userId;
    const showDate = index === 0 || formatDate(item.data_criacao) !== formatDate(mensagens[index - 1]?.data_criacao);
    const showName = !isMe && (index === 0 || mensagens[index - 1]?.remetente_id !== item.remetente_id);
          ...
          ListEmptyComponent={
            <View style={styles.emptyContainer}>
              ...
              <Text style={styles.emptySubtitle}>
                Envie uma mensagem para o RH e aguarde a resposta.
              </Text>
            </View>
          }
        />
