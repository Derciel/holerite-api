...
export default function HomeScreen() {
  ...
  const loadData = useCallback(async () => {
    if (!user?.id) return;
    const result = await fetchHolerites(user.id);
    if (result.error) {
...
const styles = StyleSheet.create({
  ...
  headerTools: { flexDirection: 'row', gap: 10 },
  toolBtn: {
    backgroundColor: 'rgba(255,255,255,0.05)', width: 40, height: 40,
  ...
  badge: { backgroundColor: 'rgba(255,255,255,0.15)', borderRadius: 50, paddingHorizontal: 7, paddingVertical: 2 },
  badgeActive: { backgroundColor: 'rgba(0,0,0,0.2)' },
  badgeText: { fontSize: 11, fontWeight: '700', color: '#888' },
  badgeTextActive: { color: '#000' },
  loader: { marginTop: 60 },
  errorContainer: { alignItems: 'center', marginTop: 60, paddingHorizontal: 24 },
  errorText: { fontSize: 14, color: '#FF5050', textAlign: 'center', marginBottom: 16 },
  retryButton: { paddingHorizontal: 24, paddingVertical: 12, backgroundColor: '#00C19C', borderRadius: 12 },
  retryText: { fontSize: 14, color: '#000', fontWeight: '700' },
  ...
  monthDivider: {
    fontSize: 12, fontWeight: '800', color: '#666', textTransform: 'uppercase',
